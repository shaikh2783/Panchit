...
class _MyCoursesPageState extends State<MyCoursesPage>
  ...
  void _onTabChanged() {
    if (_tabController.index == 0 && _enrolledCourses.isEmpty && !_isLoadingEnrolled) {
      _loadEnrolledCourses();
    } else if (_tabController.index == 1 && _createdCourses.isEmpty && !_isLoadingCreated) {
      _loadCreatedCourses();
    }
  }
  ...
  Future<void> _loadEnrolledCourses() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _enrolledError = 'فشل تحميل الدورات المسجل فيها';
          _isLoadingEnrolled = false;
        });
      }
    }
  ...
  Future<void> _loadCreatedCourses() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _createdError = 'فشل تحميل الدورات المنشأة';
          _isLoadingCreated = false;
        });
      }
    }
  ...
  List<Post> _filterEnrolled(List<Post> courses) {
    ...
    return courses.where((course) {
      final isCreator = course.course?.iOwner == true || 
  ...
  List<Post> _filterCreated(List<Post> courses) {
    ...
    return courses.where((course) {
      return course.course?.iOwner == true || 
             course.authorId == userId;
    }).toList();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Get.isDarkMode ? const Color(0xFF121212) : Colors.grey[50],
      appBar: AppBar(
        title: const Text(
          'دوراتي',
          style: TextStyle(
          ...
          IconButton(
            onPressed: () async {
              final result = await Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const CourseCreatePage(),
                ),
              );
      ...
      body: TabBarView(
        ...
        children: [
          ...
          _buildCoursesList(
            ...
            error: _createdError,
            emptyMessage: 'لم تنشئ أي دورة بعد',
