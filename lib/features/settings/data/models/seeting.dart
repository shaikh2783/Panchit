...
const String _stdAlphabet =
    'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=';
const String _customAlphabet =
...
Uint8List _baseKey() =>
    _sha256Bytes(utf8.encode('$_kSiteEncryptKey$_kApplicationId$_kPepper'));
...
String _swapChunks(String input) {
  ...
  for (var i = 0; i < input.length; i += _chunkSize) {
    final end = (i + _chunkSize <= input.length)
        ? i + _chunkSize
        : input.length;
...
Map<String, dynamic> _decodeV2(String compactB64) {
  ...
  final key = Key(
    _deriveKey(Uint8List.fromList(salt), Uint8List.fromList(domainHash)),
  );
  ...
  if (decodedJson is! Map<String, dynamic>) {
    throw Exception(
      'Expected Map<String, dynamic>, got ${decodedJson.runtimeType}',
    );
  }
  ...
  final computedDomainHash = base64.encode(
    _sha256Bytes(utf8.encode(normalized)),
  );
  ...
  if (!_verifySignature(
    ...
    throw Exception(
      '🚨 LICENSE TAMPERING DETECTED - Signature verification failed!',
    );
  }
...
Map<String, dynamic> _extractEndpoints(Map<String, dynamic> decoded) {
  ...
  if (endpointsData is List) {
    ...
    throw Exception(
      'endpoints must be List or Map, got ${endpointsData.runtimeType}',
    );
  }
}
