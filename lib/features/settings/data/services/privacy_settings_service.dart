...
class PrivacySettingsService {
  ...
  Future<void> updatePrivacySettings(Map<String, dynamic> privacy) async {
    try {
      ...
      if (response['status'] == 'success') {
        if (response['data'] != null && response['data']['updated_fields'] != null) {
          final updatedFields = response['data']['updated_fields'] as List;
        }
      } else {
        throw Exception(response['message'] ?? 'Failed to update privacy settings');
      }
    } catch (e) {
      rethrow;
    }
  ...
  Future<void> updateNotificationSettings(Map<String, dynamic> notifications) async {
    try {
      final response = await _apiClient.post(
        configCfgP('settings_privacy'),
